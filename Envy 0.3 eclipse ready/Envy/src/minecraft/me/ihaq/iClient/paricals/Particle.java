...
public class Particle {
	...
	public void render(final ArrayList<Particle> lastparticles, final int mouseX, final int mouseY) {
        ...
        final float alpha = (color.getRGB() >> 24 & 255) / 255.0f;
        final float red = (color.getRGB() >> 16 & 255) / 255.0f;
        final float green = (color.getRGB() >> 8 & 255) / 255.0f;
        final float blue = (color.getRGB() & 255) / 255.0f;
        GL11.glPushMatrix();
        GlStateManager.enableBlend();
        ...
        int i2 = 0;
        while (i2 <= 360) {
            GL11.glVertex2d((int) realX + Math.sin(i2 * 3.1415926 / 180.0) * size,
            		(int) realY + Math.cos(i2 * 3.1415926 / 180.0) * size);
            ++i2;
        }
        GL11.glEnd();
